...
- ➖ **Not needed** — the role is in a **non-empty** `authorized_in` country (or is genuinely location-agnostic remote the candidate can work from an authorized country), **or** the profile explicitly declares `needs_sponsorship: false`.
...
- Only an explicitly declared `needs_sponsorship: false` may use that profile flag for ➖ Not needed; an omitted key is not false.
- If `authorized_in`, `needs_sponsorship`, and `visa_status` are all absent or blank, eligibility is `unknown` and the work-auth tier is ⚠️ Unstated (neutral). Never guess ➖ Not needed from the candidate's country, residence, job location, or title.
- If the profile has no `authorized_in`/`needs_sponsorship` keys but has free-text `visa_status`, use it conservatively and default to ⚠️ Unstated rather than guessing a blocker.
